<?php
namespace Ess\M2ePro\Block\Adminhtml\Ebay\Listing\Grid;

/**
 * Interceptor class for @see \Ess\M2ePro\Block\Adminhtml\Ebay\Listing\Grid
 */
class Interceptor extends \Ess\M2ePro\Block\Adminhtml\Ebay\Listing\Grid implements \Magento\Framework\Interception\InterceptorInterface
{
    use \Magento\Framework\Interception\Interceptor;

    public function __construct(\Ess\M2ePro\Model\ActiveRecord\Component\Parent\Ebay\Factory $ebayFactory, \Ess\M2ePro\Block\Adminhtml\Magento\Context\Template $context, \Magento\Backend\Helper\Data $backendHelper, array $data = [])
    {
        $this->___init();
        parent::__construct($ebayFactory, $context, $backendHelper, $data);
    }
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'callbackColumnSoldQTY');
        if (!$pluginInfo) {
            return parent::callbackColumnSoldQTY($value, $row, $column, $isExport);
        } else {
            return $this->___callPlugins('callbackColumnSoldQTY', func_get_args(), $pluginInfo);
        }
    }
