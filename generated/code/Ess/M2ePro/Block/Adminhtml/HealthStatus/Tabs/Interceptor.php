...
class Interceptor extends \Ess\M2ePro\Block\Adminhtml\HealthStatus\Tabs implements \Magento\Framework\Interception\InterceptorInterface
{
    ...
    public function __construct(\Ess\M2ePro\Model\HealthStatus\Task\Result\Set $resultSet, \Ess\M2ePro\Block\Adminhtml\Magento\Context\Template $context, \Magento\Framework\Json\EncoderInterface $jsonEncoder, \Magento\Backend\Model\Auth\Session $authSession, array $data = [])
    {
        $this->___init();
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'getActiveTabById');
        if (!$pluginInfo) {
            return parent::getActiveTabById($id);
        } else {
            return $this->___callPlugins('getActiveTabById', func_get_args(), $pluginInfo);
        }
    }
